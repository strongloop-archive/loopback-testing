...
_describe.whenCalledByUser = function(credentials, verbs, url, data, cb) {
  describe('when called by logged in user', function () {
    _beforeEach.givenLoggedInUser(credentials);
    [].concat(verbs).forEach(function (verb) {
      _describe.whenCalledRemotely(verb, url, data, cb);
    });
  });
}

_describe.whenCalledByUserWithRole = function (credentials, role, verbs, url, data, cb) {
...
_describe.whenCalledAnonymously = function(verbs, url, data, cb) {
...
_describe.whenCalledUnauthenticated = function(verbs, url, data, cb) {
